...
public class Ai_Dir_7_2 : AI_Dir_Generic
{
    [Header("Level 7 Version 2")]
    ...
    public float timeTillNewEnemy = 40f;
    ...
    void Update()
    {
        ...
        //Game Flow
        gameTime = Level_Transition.gameTime;
        if (gameTime > 65)
        {
            lvl_enemies_min_spawn_rate = 2.5f;
            lvl_enemies_max_spawn_rate = 3.5f;
        }
        ...
        if (!willMakeEnemies)
        {
            if (gameTime >= 0 && gameTime < timeTillNewEnemy)
            {
                randomEnemyID = 0; //Arc Throwing Enemies
                lvl_enemies_spawn_location_y_offset = Random.Range(-4.5f, 4.5f);
            }
            else if (gameTime >= timeTillNewEnemy && gameTime < (timeTillNewEnemy * 2) - 5)
            {
            ...
            else
            {
                ...
                lvl_enemies_spawn_location_y_offset = Random.Range(-5.0f, 5.0f);
            }
